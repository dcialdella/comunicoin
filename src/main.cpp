...
// Copyright (c) 2013-2014 Pesetacoin Developers (CryptoMP & Rampa)
...
uint256 hashGenesisBlock("0xedfe5830b53251bfff733600b1cd5c192e761c011b055f07924634818c906438");

//uint256 hashGenesisBlock("0x89a47c0df0ab17773b26d2f03a480eb2a11bc022e83e611ca14b88428e0f4252");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pesetacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PesetaCoin Signed Message:\n";
...
{
    // Pesetacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 12 * 60 * 60; // 3.5 days
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pesetacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
// CryptoMP - Added KimotoGravityWell 3 Functions and adjusted 60secs block target
...
{
        static const int64        BlocksTargetSpacing                        = 1 * 60; // 60 seconds PTC Block Target //CryptoMP
...
bool CBlock::CheckBlock(CValidationState &state, int nHeight, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pesetacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Pesetacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        const char* pszTimestamp = "Genesis born 12years after pesetas death";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xd2a85c7a8606d7c234b366bf831d37d71a2a061f0e95ff1246fa19a80a1f5a16"));
...
unsigned char pchMessageStart[4] = { 0xc0, 0xc0, 0xc0, 0xc0 }; // Pesetacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PesetacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PesetacoinMiner : ProcessBlock, block not accepted");
    }
